...
export class JoinBannerService {
  ...
  async create(input: JoinBannerInput): Promise<Buffer> {
    ...
    const textLayer = createTextLayer(input, template.width, template.height, avatarSize, avatarTop)
...
function estimateTextUnits(text: string): number {
  return Array.from(text).reduce((total, character) => total + estimateCharacterUnits(character), 0)
}
