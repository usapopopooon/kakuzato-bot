# コマンド一覧

すべてのコマンドはサーバー内専用です。実行にはサーバーの管理者権限が必要です。

## `/activity`

Bot の「プレイ中」表示を管理します。設定は Bot 全体で共通です。

| コマンド           | 引数   | 説明                                                            |
| ------------------ | ------ | --------------------------------------------------------------- |
| `/activity set`    | `name` | Bot のプレイ中表示を設定し、即時反映します。最大 128 文字です。 |
| `/activity status` | なし   | 現在のプレイ中表示を確認します。                                |
| `/activity reset`  | なし   | プレイ中表示をデフォルトの `サーバーを管理中。` に戻します。    |

設定は PostgreSQL に保存されます。

## `/welcome`

サーバー参加時に送信する welcome 画像と本文を管理します。

| コマンド                  | 引数      | 説明                                                                 |
| ------------------------- | --------- | -------------------------------------------------------------------- |
| `/welcome set`            | `channel` | welcome 画像の送信先チャンネルを設定し、welcome 投稿を有効にします。 |
| `/welcome message`        | `content` | welcome 画像と一緒に送る本文を設定します。最大 1500 文字です。       |
| `/welcome banner-message` | `content` | welcome 画像内に表示するメッセージを設定します。最大 160 文字です。  |
| `/welcome status`         | なし      | welcome 投稿の有効状態、送信先、本文、画像内メッセージを確認します。 |
| `/welcome test`           | なし      | 設定済みチャンネルにテスト投稿します。                               |
| `/welcome disable`        | なし      | welcome 投稿を無効にします。                                         |

`/welcome message` と `/welcome banner-message` で使えるプレースホルダー:

| プレースホルダー | 内容                              |
| ---------------- | --------------------------------- |
| `{mention}`      | 参加したユーザーの通知なし @ 表示 |
| `{username}`     | 参加したユーザーのユーザー名      |
| `{displayName}`  | サーバー内での表示名              |
| `{guildName}`    | サーバー名                        |
| `{memberCount}`  | 現在のメンバー数                  |

本文が Discord のメッセージ上限を超える場合は、送信時に 2000 文字以内へ省略されます。設定は PostgreSQL に保存されます。

## `/note`

メンバーごとのノートチャンネルを作成するパネルを管理します。管理者がロビーにパネルを設置すると、一般メンバーはボタン操作だけでノートを作成・管理できます。

| コマンド        | 引数                                                                            | 説明                                                                                             |
| --------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `/note setup`   | `lobby`, `category_base_name`, `channel_prefix`, `creator_role`, `manager_role` | ノート作成パネルを設置します。ロビーはメンバー投稿・スレッド作成・リアクション不可に更新します。 |
| `/note status`  | なし                                                                            | ロビー、作成可能ロール、管理カテゴリ、ノート数を表示します。                                     |
| `/note disable` | なし                                                                            | ノート作成パネル設定を解除します。既存のノートチャンネルは残ります。                             |

パネルからできる操作:

| 場所          | 操作                | 説明                                                                       |
| ------------- | ------------------- | -------------------------------------------------------------------------- |
| ロビー        | ノートを作る / 開く | 未作成なら公開・コメント可のノートを作成し、作成済みならリンクを返します。 |
| ロビー        | 閉じたノートを復元  | アーカイブカテゴリに移動したノートを通常カテゴリへ戻します。               |
| ロビー/ノート | ノート名を変える    | 入力画面からノートチャンネル名を変更します。                               |
| ノート        | 公開 / 非公開       | サーバーメンバー全員に見える状態と本人のみの状態を切り替えます。           |
| ノート        | コメント可否        | 公開状態のまま、他メンバーの投稿可否を切り替えます。                       |
| ノート        | ユーザーをブロック  | 選択したユーザーを自分のノートから見えない状態にします。                   |
| ノート        | ブロック解除        | 選択したユーザーの個別ブロックを解除します。                               |
| ノート        | 閉じる              | ノートをアーカイブカテゴリへ移動し、本人からも非表示にします。             |

`creator_role` は任意です。指定した場合だけ、そのロールを持つメンバーに新規ノート作成を限定します。未指定の場合はロール制限なしで作成できます。既に作成済みのノートを開く操作はそのまま使えます。

ノートを持つメンバーがサーバーから抜けた場合、そのノートは自動で閉じたノートとしてアーカイブされます。再参加後はロビーの「閉じたノートを復元」から戻せます。

作成時のチャンネル名は `表示名のノート` になります。英字は Discord のチャンネル名に合わせて小文字化され、空白はハイフンに置き換えられます。表示名から使える文字が残らない場合だけ、`channel_prefix` を使った名前にフォールバックします。

ノート内の管理ボタンは作成者だけが使えます。管理者、Bot、`manager_role` のメンバーはブロック対象にできません。

ノートカテゴリは 50 チャンネルごとに自動で増えます。たとえば `ノート` が埋まると `ノート 2` が作成されます。閉じたノートも `ノート Archive`、`ノート Archive 2` のように管理されます。

## `/post-role`

指定チャンネルに一度でも投稿したメンバーへロールを付与します。付与済みメンバーへの再付与はスキップするため、同じ投稿や同期を何度処理しても安全です。

| コマンド            | 引数                               | 説明                                                                               |
| ------------------- | ---------------------------------- | ---------------------------------------------------------------------------------- |
| `/post-role setup`  | `channel`, `role`, `history_limit` | 対象チャンネルと付与ロールを設定し、初回の履歴同期も実行します。                   |
| `/post-role remove` | `channel`                          | 指定チャンネルの設定を解除します。既に付与済みのロールは残ります。                 |
| `/post-role status` | なし                               | 投稿実績ロールの設定一覧を表示します。                                             |
| `/post-role sync`   | `channel`                          | 過去ログを再確認してロール付与を実行します。`channel` 省略時は全設定を同期します。 |

Bot 起動時にも設定済みチャンネルの過去ログを確認し、Bot 停止中の投稿を拾います。確認件数は `history_limit` で指定でき、省略時は 500 件、最大 5000 件です。

対象は通常のテキストチャンネルまたはアナウンスチャンネルです。Bot には対象チャンネルの閲覧・メッセージ履歴閲覧権限と、付与ロールを管理できるロール位置・ロール管理権限が必要です。

## `/auto-reaction`

指定チャンネルに新しく投稿されたメッセージへ、設定したリアクションを自動で付けます。

| コマンド                | 引数                | 説明                                                                     |
| ----------------------- | ------------------- | ------------------------------------------------------------------------ |
| `/auto-reaction setup`  | `channel`, `emojis` | 対象チャンネルと付与するリアクションを設定します。                       |
| `/auto-reaction remove` | `channel`           | 指定チャンネルの自動リアクション設定を解除します。                       |
| `/auto-reaction status` | `channel`           | 自動リアクション設定を表示します。`channel` 省略時は全設定を表示します。 |

`emojis` はスペースまたはカンマ区切りで最大 20 個まで指定できます。Unicode 絵文字とカスタム絵文字表記をそのまま使えます。

対象は通常のテキストチャンネルまたはアナウンスチャンネルです。Bot には対象チャンネルの閲覧権限とリアクション追加権限が必要です。設定は PostgreSQL に保存されます。

## `/sticky`

チャンネルの最新位置に固定表示される sticky メッセージを管理します。対象チャンネルに新しい投稿があると、指定秒数後に古い sticky メッセージを削除し、同じ内容を再投稿します。

| コマンド         | 引数                       | 説明                                                                                               |
| ---------------- | -------------------------- | -------------------------------------------------------------------------------------------------- |
| `/sticky text`   | `channel`, `delay_seconds` | テキスト形式の sticky メッセージを設定します。本文は表示される入力画面で複数行入力できます。       |
| `/sticky embed`  | `channel`, `delay_seconds` | Embed 形式の sticky メッセージを設定します。タイトル、色、説明文は表示される入力画面で指定します。 |
| `/sticky status` | `channel`                  | sticky メッセージ設定を確認します。                                                                |
| `/sticky remove` | `channel`                  | sticky メッセージを解除し、現在表示中の sticky メッセージも削除します。                            |

sticky メッセージ本文と Embed 説明文は、入力画面のテキストエリアでそのまま複数行にできます。

`delay_seconds` は 1 秒から 3600 秒まで指定できます。省略時は 5 秒です。

`/sticky embed` の色は入力画面で `FF0000`、`#00FF00`、`0x3366FF` の形式で指定できます。設定は PostgreSQL に保存されます。

## `/bump`

DISBOARD とディス速報の bump 成功を検知し、サービスごとに設定した時間後にリマインドします。デフォルトは DISBOARD が 5 時間後、ディス速報が 2 時間後です。

| コマンド        | 引数                 | 説明                                                                       |
| --------------- | -------------------- | -------------------------------------------------------------------------- |
| `/bump setup`   | なし                 | 実行したチャンネルを bump 監視チャンネルに設定します。履歴同期も行います。 |
| `/bump status`  | なし                 | 監視チャンネル、サービス別の通知状態、通知ロール、次回時刻を表示します。   |
| `/bump sync`    | なし                 | 監視チャンネル履歴から前回 bump を判定して次回通知を設定します。           |
| `/bump delay`   | `service`, `minutes` | サービスごとのリマインド時間を分単位で設定します。                         |
| `/bump disable` | なし                 | bump 監視とリマインダーを停止します。                                      |

通知メッセージや `/bump status` に表示されるボタンから、サービスごとの通知 ON/OFF と通知先ロールを変更できます。通知先ロール未設定時はメンションなしで、ロールを設定した場合のみそのロールにメンションします。設定、リマインド時間、リマインダーは PostgreSQL に保存されます。

## `/voice-notify`

指定した VC の入退室を、指定チャンネルへ短文で通知します。

| コマンド               | 引数              | 説明                                         |
| ---------------------- | ----------------- | -------------------------------------------- |
| `/voice-notify add`    | `voice`, `notify` | 監視する VC と通知先チャンネルを設定します。 |
| `/voice-notify remove` | `voice`           | 指定 VC の入退室通知を解除します。           |
| `/voice-notify status` | なし              | VC 入退室通知の設定一覧を表示します。        |

通知本文は `表示名 さんが #VC名 に入室しました。` / `表示名 さんが #VC名 から退室しました。` の形式です。設定は PostgreSQL に保存されます。

## `/automod`

サーバー参加時の AutoMod を管理します。アバター未設定ユーザーと、作成から指定期間未満のアカウントを検知し、BAN/KICK/タイムアウトを実行できます。

| コマンド                       | 引数                                   | 説明                                                                  |
| ------------------------------ | -------------------------------------- | --------------------------------------------------------------------- |
| `/automod log set`             | `channel`                              | AutoMod 実行ログの送信先チャンネルを設定します。                      |
| `/automod log disable`         | なし                                   | AutoMod 実行ログのチャンネル送信を無効にします。DB 保存は継続します。 |
| `/automod no-avatar set`       | `action`, `timeout_minutes`            | アバター未設定ユーザーへの AutoMod を有効にします。                   |
| `/automod no-avatar disable`   | なし                                   | アバター未設定ルールを無効にします。                                  |
| `/automod account-age set`     | `minutes`, `action`, `timeout_minutes` | 作成から指定分数未満のアカウントへの AutoMod を有効にします。         |
| `/automod account-age disable` | なし                                   | アカウント作成期間ルールを無効にします。                              |
| `/automod status`              | なし                                   | AutoMod のログ送信先とルール設定を確認します。                        |

`action` は `BAN`、`KICK`、`タイムアウト` から選択します。`timeout_minutes` は action がタイムアウトの場合だけ使われ、省略時は 60 分です。

設定時に、選択した action に必要な Bot 権限も確認します。AutoMod の設定、ルール、実行ログは PostgreSQL に保存されます。実行ログには成功/失敗ステータスも記録されます。

## `/eventlog`

サーバー内イベントのログ送信を管理します。

| コマンド             | 引数                  | 説明                                                                 |
| -------------------- | --------------------- | -------------------------------------------------------------------- |
| `/eventlog set`      | `channel`             | イベントログの送信先チャンネルを設定し、イベントログを有効にします。 |
| `/eventlog category` | `category`, `enabled` | カテゴリ単位でイベントログの送信を切り替えます。                     |
| `/eventlog status`   | なし                  | イベントログの有効状態、送信先、有効カテゴリを確認します。           |
| `/eventlog test`     | なし                  | 設定済みチャンネルにテストログを送信します。                         |
| `/eventlog disable`  | なし                  | イベントログを無効にします。                                         |

`/eventlog category` のカテゴリ:

| 値           | 表示名         |
| ------------ | -------------- |
| `message`    | メッセージ     |
| `member`     | メンバー       |
| `moderation` | モデレーション |
| `server`     | サーバー変更   |
| `voice`      | ボイス         |

設定は PostgreSQL に保存されます。

## 初期設定例

```text
/activity set name:サーバーを管理中。
/welcome set channel:#welcome
/welcome message content:Welcome, {mention}!
/welcome banner-message content:{displayName} さんが召喚されました！
/welcome test
/sticky text channel:#notice
/post-role setup channel:#self-intro role:@投稿済み
/auto-reaction setup channel:#notice emojis:👍 ❤️
/bump setup
/voice-notify add voice:#general notify:#vc-log
/automod log set channel:#server-log
/automod account-age set minutes:1440 action:BAN
/eventlog set channel:#server-log
/eventlog test
```
