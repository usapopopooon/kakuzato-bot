...
| `/welcome banner-message` | `content` | welcome 画像内に表示するメッセージを設定します。最大 160 文字です。  |
| `/welcome status`         | なし      | welcome 投稿の有効状態、送信先、本文、画像内メッセージを確認します。 |
| `/welcome test`           | なし      | 設定済みチャンネルにテスト投稿します。                               |
| `/welcome disable`        | なし      | welcome 投稿を無効にします。                                         |

`/welcome message` と `/welcome banner-message` で使えるプレースホルダー:
...
DISBOARD とディス速報の bump 成功を検知し、サービスごとに設定した時間後にリマインドします。デフォルトは DISBOARD が 5 時間後、ディス速報が 2 時間後です。
...
| `/bump delay`   | `service`, `minutes` | サービスごとのリマインド時間を分単位で設定します。             |
| `/bump disable` | なし | bump 監視とリマインダーを停止します。                                      |

通知メッセージや `/bump status` に表示されるボタンから、サービスごとの通知 ON/OFF と通知先ロールを変更できます。通知先ロール未設定時はメンションなしで、ロールを設定した場合のみそのロールにメンションします。設定、リマインド時間、リマインダーは PostgreSQL に保存されます。
...
/welcome banner-message content:{displayName} さんが召喚されました！
/welcome test
