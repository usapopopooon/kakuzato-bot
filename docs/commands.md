...
| コマンド         | 引数                                                        | 説明                                                                                       |
| ---------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `/sticky text`   | `channel`, `content`, `delay_seconds`                       | テキスト形式の sticky メッセージを設定します。`delay_seconds` は任意です。                 |
| `/sticky embed`  | `channel`, `description`, `title`, `color`, `delay_seconds` | Embed 形式の sticky メッセージを設定します。`title`, `color`, `delay_seconds` は任意です。 |
| `/sticky status` | `channel`                                                   | sticky メッセージ設定を確認します。                                                        |
| `/sticky remove` | `channel`                                                   | sticky メッセージを解除し、現在表示中の sticky メッセージも削除します。                    |

`content` と `description` は `\n` を使って複数行にできます。例: `1行目\n2行目`

`delay_seconds` は 1 秒から 3600 秒まで指定できます。省略時は 5 秒です。

`/sticky embed` の `color` は `FF0000`、`#00FF00`、`0x3366FF` の形式で指定できます。設定は `data/sticky-message-configs.json` に保存されます。
...
/welcome test
/sticky text channel:#notice content:この案内は常に最新に表示されます。
